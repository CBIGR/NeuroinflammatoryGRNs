...
sampleName <- "HPU_counts_MDD" #Change for this analysis!!!
sampleFolder<-"/scratch/gent/vo/000/gvo00027/projects/CBIGR/21JDS_singlecell_GBM/SCENIC/SCENIC_vsn/results/HPU_SCENIC_thesis/counts_MDD/"

##add some subfolders
dir.create(paste0(sampleFolder,"results/"))

counts_MDD <- fread("/kyukon/scratch/gent/vo/000/gvo00027/projects/CBIGR/21HPU_GRN_neuroinfl/singlecell/SCENIC/input/counts_MDD.txt", data.table=FALSE)
rownames(counts_MDD) <- counts_MDD[,1]
counts_MDD[,1] <- NULL
print(counts_MDD[1:5,1:5])
seuratObj <- CreateSeuratObject(counts_MDD)
##### Read object --> JDS: same object as given as input to SCENIC
celltypes <- str_extract(rownames(seuratObj@meta.data), "[A-Za-z]+(?=_|\\.)")
names(celltypes) <- rownames(seuratObj@meta.data)
...
#seuratObj <- SetIdent(seuratObj, value = "cell_state") ## JDS: added to plot cell states in heatmaps 
cellInfo <- as.data.frame(seuratObj@active.ident)
dim(cellInfo)
#1386    1

# Read data from loom file
loom <- open_loom(file.path = paste0(sampleFolder,"counts_MDD/scenic/counts_MDD/SCENIC_output.loom"))
