---
title: "Post processing of SCENIC analysis on MDD dataset - old runs made by Joke"
...
Import metadata: 
```{r}
metaData = read.table("Tables/Metadata_MDD.txt", header = T, sep = "\t")
head(metaData)
```
...
loom <- open_loom(file = "~/Desktop/rebuttal_Hanne/SCENIC_output_MDD_large.loom")
...
write.csv( edges, paste0(table_dir, "Edges_MDD_large.csv"), quote = F)
```

```{r}
# Create edge list restricted for DEGs

degs <- readr::read_csv("Tables/diff_exp_celltypes_MDDvsC.csv")
head(degs)
```

```{r}
edges_degs<-edges[edges$unlist.regulons. %in% degs[startsWith(degs$celltype, "MICRO"), ]$genes ,] # for microglia
dim(edges_degs)
```
...
ggplot(df_plot, aes(x=Rank, y=Degree)) +
  ...
  theme_bw() +
  ggtitle("Degree of TFs regulating DEGs in MDD vs control")

ggsave(paste0(fig_dir, "MDD_GRN_fromDEGs_MICRO_degreeRankPlot.png"))
ggsave(paste0(fig_dir, "MDD_GRN_fromDEGs_MICRO_degreeRankPlot.svg"))
```

```{r}
# Repeat for all DEGs
edges_degs<-edges[edges$unlist.regulons. %in% degs$genes ,] 
dim(edges_degs)
```
...
write.csv(df_plot[df_plot$TF %in% crc_tfs,], paste0(table_dir, "MDD_GRN_fromDEGs_ALL_degreeRanks_CRCs.csv"), row.names = F, quote = F)
```
...
ggplot(df_plot, aes(x=Rank, y=Degree)) +
  ...
  ggrepel::geom_label_repel(aes(label=ifelse(Rank<=20,as.character(TF),'')),
                  ...
                  max.overlaps = Inf) +
  theme_bw() +
  ggtitle("Degree of TFs regulating DEGs in MDD vs control - all cells")

ggsave(paste0(fig_dir, "MDD_GRN_fromDEGs_ALL_degreeRankPlot.png"))
ggsave(paste0(fig_dir, "MDD_GRN_fromDEGs_ALL_degreeRankPlot.svg"))
```
...
write.xlsx(tmpList,paste0(table_dir, "tableRegulons_MDD_large.xlsx"))
```
...
saveRDS(AUCdata,"Tables/AUCdata_MDD_large.rds")
saveRDS(regulonSelection, "Tables/RegSelections_MDD_large.rds")
```
...
for (i in seq_len(length(regulonSelection))) {
  ...
  if(length(regulonSelection[[selRegs]])>1) {
    ...
    file_name <- paste0(fig_dir, "Heatmap_AUCRegulons_Averaged_", selRegs ,"_MDDlarge.pdf")
...
for (i in seq_len(length(regulonSelection))) {
  ...
  if(length(regulonSelection[[selRegs]])>1) {
    ...
    file_name <- paste0(fig_dir, "Heatmap_AUCRegulons_Averaged_Diagnosis_", selRegs ,"_MDDlarge.pdf")
...
for(i in seq_len(length(regulonSelection))){ 
  ...
  if(length(regulonSelection[[selRegs]])>1) {
    ...
    pdf(paste0(fig_dir, "RegulonActivity_Heatmap_",selRegs,"_ComplexHeatmap_MDDlarge.pdf"), width=12.45, height=17.55)
...
### Test activity of TFs of interest

```{r}
AUCdata <- readRDS("Tables/AUCdata_MDD_large.rds")
dim(AUCdata)
```
...
so@meta.data<- metaData[,1:5]

so <- SetIdent(so, value="Celltype")

so_sub <- subset(so, idents="MICRO")
so_sub
```
...
```{r}
wilc_mg <- wilc_mg[wilc_mg$group == "Suicide" & wilc_mg$padj < 0.05,]
write.csv(wilc_mg[order(wilc_mg$logFC, decreasing = T),], paste0(table_dir, "TestWilc_MICRO_MDDlarge.csv"), quote = F)
```
...
```{r}
df_plot$Cell <- row.names(metaData)
...
ggsave(paste0(fig_dir, "Boxplot_MDDlarge_TFsOI_byCondition.png"))
ggsave(paste0(fig_dir, "Boxplot_MDDlarge_TFsOI_byCondition.svg"))
ggsave(paste0(fig_dir, "Boxplot_MDDlarge_TFsOI_byCondition.pdf"))
...
for (ct in levels(as.factor(metaData$Celltype))){
  ...
  ggsave(paste0(fig_dir, "Boxplot_MDDlarge_TFsOI_", ct,"_byCondition.png"), p)
  ggsave(paste0(fig_dir, "Boxplot_MDDlarge_TFsOI_", ct,"_byCondition.svg"), p)
  ggsave(paste0(fig_dir, "Boxplot_MDDlarge_TFsOI_", ct,"_byCondition.pdf"), p)
}
```
