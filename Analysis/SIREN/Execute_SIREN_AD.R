...
counts_AD_GS <- read.delim("~/MyFiles/Master/Processed_counts_AD_GS.txt")
counts_AD_GS <- rownames_to_column(counts_AD_GS)
netw_AD <- read.delim("~/MyFiles/Master/ensemble_netw_AD.txt")
netw_AD <- netw_AD %>% select(TF, Target_gene)

# need to change the gene name to the number in the expression matrix 
df <- data.frame()
for (TF in netw_AD$TF) {
  df_TF <- counts_AD_GS %>% filter(GeneSymbol == TF) %>% select(rowname)
  df <- rbind(df, df_TF)
}
names(df) <- "TF_row"
df2 <- data.frame()
for (gene in netw_AD$Target_gene) {
  df_TG <- counts_AD_GS %>% filter(GeneSymbol == gene) %>% select(rowname)
  df2 <- rbind(df2, df_TG)
}
...
counts_AD <- counts_AD_GS %>% select(-rowname, -GeneSymbol)
counts_AD <- as.matrix(counts_AD)
head(counts_AD)
...
Result_f <- cbind(Result_f, netw_AD)
head(Result_f)
Result_f <- Result_f %>% select(TF, Target_gene, score)

Result_f <- Result_f %>% mutate(Sign = case_when(score < 0 ~ "Inhibition", 
                                          score > 0 ~ "Activation",
                                          score == 0 ~ "Not determinable"))
Result_f %>% group_by(Sign) %>% summarise(n())

write.table(Result_f, file = 'Result_f_AD.txt', sep = "\t", quote = F, row.names = F)

Result_f <- read.delim("Result_f_AD.txt")

# Format to modules
modulesAD <- read.table("~/MyFiles/Master/kmed_modules_AD.txt")
names(modulesAD) <- c("Gene", "Module")
reg_AD <- read.table("~/MyFiles/Master/reg_clust_ens_AD.txt")
names(reg_AD) <- c("TF", "Module")

modules_netw <- merge(reg_AD, modulesAD, by = "Module")
...
write.table(summarised, file = "TF_module_sign_AD.txt", row.names = F, quote = F, sep = "\t", col.names = T)
